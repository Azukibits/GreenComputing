...
GreenComputing 是一个用于估算 C++ 函数级能耗和碳排放的静态分析工具。项目包含 macOS 图形界面和跨平台命令行工具，可以帮助你快速发现代码中的相对碳排放热点。
...
Windows 和 Linux 构建命令行工具：

```bash
cmake -S . -B build
cmake --build build --target GreenComputingCLI --config Release
```
...
- macOS 包：图形界面 `GreenComputing` 和命令行工具 `GreenComputingCLI`
- Linux 包：命令行工具 `GreenComputingCLI`
- Windows 包：命令行工具 `GreenComputingCLI.exe`
